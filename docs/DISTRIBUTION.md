...
Plugin `0.2.5` is thin: it resolves `agent-delegator` from `PATH`, requires core CLI
`0.1.0-alpha.8`, runs `doctor`, and gives one exact installation instruction when versions differ.
