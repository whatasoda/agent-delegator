...
Plugin `0.2.4` is thin: it resolves `agent-delegator` from `PATH`, requires core CLI
`0.1.0-alpha.7`, runs `doctor`, and gives one exact installation instruction when versions differ.
