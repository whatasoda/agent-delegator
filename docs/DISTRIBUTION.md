...
Plugin `0.2.6` is thin: it resolves `agent-delegator` from `PATH`, requires core CLI
`0.1.0-alpha.9`, runs `doctor`, and gives one exact installation instruction when versions differ.
